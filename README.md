...
| INCENTIVE TESTING STANDARD — two-sided OCV application for offers: pin V/C by design, B as discriminator, observed-state vs propensity targeting (base-rate arithmetic), category entry point mapping (derive the events; retrieval vs conversion boundary), pre-registered decision rules, cheapest-alternative benchmark, calibration loop, lint rules I1–I13 | `incentive-testing-standard.md` | reference |
