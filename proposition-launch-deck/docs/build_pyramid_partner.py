# -*- coding: utf-8 -*-
# Logic diagram for the partner-decision archetype (#2):
# "Should we partner to launch this specific feature?"
# One question (yes-if-bounded), one answer, three reasons:
# worth launching now / the route wins the money / the deal is bounded.
W = 780
M = 22
BODY_W = W - 2*M
...
# ---------- INTRO ----------
intro = [("SITUATION", ["Invoice finance cleared the demand gate and",
                        "holds slot 2. We can build, buy, or partner."]),
         ("COMPLICATION", ["Built in-house it takes 18 months — after the",
                           "renewal window in which exporters switch.",
                           "Partnering trades margin share and a moat risk",
                           "for speed. On instinct, that trade goes unpriced."]),
         ("QUESTION", ["Should we partner to launch invoice finance —",
                       "and on what terms?"])]
...
# ---------- ANSWER ----------
ans = ["Partner with [Lender Co]: live in 5 months,",
       "£1.6m NPV — build misses the window;",
       "the terms keep the advantage ours."]
h = 34 + 30*len(ans) + 18
...
# ---------- KEY LINES ----------
keys = [
 ("KEY LINE 1 · WORTH LAUNCHING NOW", ["Demand is proven — and the", "window closes in two quarters"],
  [("OCV 72 against a gate of 60 — slot 2 of the backlog", None),
   ("Two-thirds of renewal dates fall in the next two quarters", None),
   ("Arriving at month 18 halves the modelled uptake", None),
   ("The £1.0m gap is a timing problem, not a build problem", None)]),
 ("KEY LINE 2 · THE ROUTE WINS THE MONEY", ["Partnering returns £1.0m more", "than building — all routes priced"],
  [("Partner £1.6m · white-label £1.1m · floor £0.9m · build £0.6m", None),
   ("Build wins on margin (£520 vs £390 a user) — loses on time", None),
   ("Named partner, diligenced — negatives stated, not hidden", None),
   ("Crossover computed: capex ÷ margin delta ≈ 5,000 users", "k2s3"),
   ("Do-nothing is priced too — the slot's next best use", None)]),
 ("KEY LINE 3 · THE DEAL IS BOUNDED", ["The terms keep the", "advantage ours"],
  [("We keep the customer, the data, the pricing, the brand", None),
   ("Exclusivity · data on exit · 90-day portability · re-opener", None),
   ("Month-2 milestone gate: slip and the deal re-opens", "k3s1"),
   ("Re-internalise at ~5,000 users, checked quarterly", None)]),
]
...
gutter_arrow(marks["k2s3"], marks["k3s1"] + 30, "sets the trigger")

y = connector(y, 30)

# ---------- SO WHAT ----------
sw = ["Sign the bounded terms. Fund the integration.",
      "Hold slot 2. Diarise the crossover check."]
h = 34 + 27*len(sw) + 16
...
svg = (f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" '
       ...
       + "".join(parts) + '</svg>')
open("docs/pyramid-partner.svg","w").write(svg)
print("svg", W, H)
