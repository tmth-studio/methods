/* Proposition backlog template — archetype #6: "What do we build, in what order?"
...
{
  const s = slide(true);
  s.addText("PROPOSITION BACKLOG RECOMMENDATION", { x:M, y:1.55, w:8.6, h:0.34,
    fontFace:FB, fontSize:12, bold:true, color:GOLD, charSpacing:2.6, margin:0, valign:"middle" });
  s.addText("What do we build,\nand in what order?", { x:M, y:2.05, w:8.4, h:2.1, fontFace:FH,
    fontSize:50, bold:true, color:WHITE, lineSpacing:56, margin:0, valign:"top" });
  s.addText("A rule that sorts the backlog · this cycle's slate · a re-score cadence", { x:M, y:4.28, w:8.6, h:0.4,
  ...
  note(s, "Cover. Archetype #6: the prioritisation question. The answer is a rule plus a sorted backlog, not a one-off verdict. The whole deck is a worked example - exporting SMEs, dummy numbers - so the client can see exactly what to replace.");
}
...
/* ---------------- 2b. COPILOT INSTRUCTIONS ---------------- */
{
  ...
  s.addText([
    ...
    ["6. ", "Colour roles \u2014 map, do not improvise: slate 2F4858 (covers, dividers, answer) \u2192 client primary dark \u00b7 gold C9A227 (numbers, eyebrows, accents) \u2192 client accent \u00b7 blue 4E7C90 (secondary, charts) \u2192 client secondary \u00b7 keep the light card tint logic."],
    ["7. ", "Fonts: Cambria headings \u2192 client heading font; Calibri body \u2192 client body font. Keep the sizes."],
  ...
  s.addText("Final self-check: report every example value still present, and PASS or FAIL for each reconcile check in step 2. Do not declare the deck finished with an open FAIL.",
    { x:M+0.42, y:6.45, w:CW-1.0, h:0.55, fontFace:FB, fontSize:11.5, bold:true, color:"5C4E28", margin:0, valign:"middle" });
...
/* ---------------- 4. RECOMMENDATION ---------------- */
{
  ...
  s.addText("Fill 'Trade Ready' in return order: FX and invoice finance take this cycle's two slots — £6m a year by year 3.",
    { x:M, y:1.30, w:CW, h:1.5, fontFace:FH, fontSize:28, bold:true, color:WHITE, margin:0, valign:"top", lineSpacing:36 });
  const lines = [
  ...
  let y = 3.05;
  lines.forEach((l)=>{
    s.addText(l[0], { x:M, y:y, w:2.3, h:0.56, fontFace:FH, fontSize:20, bold:true,
      color:GOLD, margin:0, valign:"middle" });
    s.addText(l[1], { x:M+2.4, y:y, w:6.6, h:0.56, fontFace:FH, fontSize:19, bold:true,
      color:WHITE, margin:0, valign:"middle" });
    s.addText(l[2], { x:M+9.1, y:y+0.04, w:3.3, h:0.5, fontFace:FB, fontSize:13,
      color:BLUEL, margin:0, valign:"middle" });
    y += 0.78;
  });
  s.addShape(p.ShapeType.line, { x:M, y:6.30, w:CW, h:0, line:{ color:"3E6072", width:1 } });
  s.addText("The client approves two things: this cycle's slate, and the rule that produced it. Next cycle re-runs the rule.",
    { x:M, y:6.48, w:CW-0.8, h:0.6, fontFace:FB, fontSize:14, color:BLUEL, margin:0, valign:"top", lineSpacing:20 });
  note(s, "The governing thought is one sentence: rule + first output + so-what. The four lines below expand it without adding new claims - each maps to a section of the deck.");
}
...
/* ---------------- 12. THE SORT ---------------- */
{
  ...
  card(s, M, y+0.18, CW, 1.35, "F3EEDD");
  ...
  s.addText("Insurance beats invoice finance on raw NPV (£1.8m vs £1.6m) — and still sorts below it, because it eats both of the cycle's slots. Capacity is the scarce thing; returns are measured against what they consume.",
    { x:M+0.42, y:y+0.66, w:CW-1.0, h:0.75, fontFace:FB, fontSize:13, color:"5C4E28", margin:0, valign:"top", lineSpacing:18 });
...
/* ---------------- 21. SO WHAT ---------------- */
{
  ...
  asks.forEach((a,i)=>{
    ...
    s.addText(a[0], { x:M+1.05, y:y, w:1.6, h:0.88, fontFace:FH, fontSize:19, bold:true,
      color:GOLD, margin:0, valign:"middle" });
    s.addText(a[1], { x:M+2.7, y:y, w:6.3, h:0.88, fontFace:FB, fontSize:14,
  ...
  note(s, "Four asks: the rule, the slate, the terms, the stop. Adopting the rule is the ask that outlives the meeting.");
}
...
p.writeFile({ fileName: "proposition-backlog-template.pptx" })
